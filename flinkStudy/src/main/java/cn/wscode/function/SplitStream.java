 ...
 * @title: SplitStream
 * @projectName DS
 * @description: TODO 分流使用侧输出流实现，减少数据的复制
 * @date 2022/4/6 11:10
 */
public class SplitStream {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<Event> dataSource = env.addSource(new CustomSource())
                .assignTimestampsAndWatermarks(WatermarkStrategy.<Event>forBoundedOutOfOrderness(Duration.ZERO)
                        .withTimestampAssigner((SerializableTimestampAssigner<Event>) (element, recordTimestamp) -> element.timestamp));


        SingleOutputStreamOperator<Event> process = dataSource.process(new ProcessFunction<Event, Event>() {
            @Override
            public void processElement(Event value, Context ctx, Collector<Event> out) throws Exception {
                if (value.name.equals("Mary")) {
                    ctx.output(MaryTag, new Tuple3<>(value.name, value.url, value.timestamp));
                } else if (value.name.equals("Bob")) {
                    ctx.output(BobTag, new Tuple3<>(value.name, value.url, value.timestamp));
                } else {
                    out.collect(value);
                }
            }
